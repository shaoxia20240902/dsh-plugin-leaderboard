...
`.trim();
		...
		function PluginRow({ item, t }) {
			...
			const copy = async () => {
				if (!await writeClipboard(item.install)) return;
				setCopied(true);
				window.setTimeout(() => {
					setCopied(false);
				}, 1600);
				...
				children: [open && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("footer", {
							...
							children: [state.status === "ready" && state.snapshot.incomplete ? `${translate("incomplete")} ` : "", translate("heatHint")]
						})
					]
		...
		const zh = {
			...
			refresh: "刷新",
			sample: "样本 {total} 个仓库",
			heatHint: "最火按星标密度和近期活跃度取 Top 10。第三方代码，安装前请阅读源码并钉住提交。",
		...
		const en = {
			...
			heatHint: "On fire is the top 10 by star density and recent activity. Third-party code — review the source and pin a commit.",
